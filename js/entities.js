...
// Registration and anatomical scale are prepared by tools/hero-delivery.cjs.
const HERO_REG = {};
const HERO_CELL = {
...
const HERO_CELLS = 24;
// All replacement locomotion states are already right-facing.
const HERO_CELL_MIRROR = {}; // Every replacement locomotion frame faces right.
// Shared delivery scale preserves the approved guard's world-space size.
const HERO_DELIVERY_K = 1.441025641025641;
const HERO_POSE_K = {};
...
const HERO_TRANS = {
  ...
  land: { key: 'transLand', cells: 4, k: HERO_DELIVERY_K, t: p => p.landT, t0: p => p.land0 || 0.12 },
  ...
  skid: { key: 'transSkid', cells: 4, k: HERO_DELIVERY_K, t: p => p.skidT, t0: p => p.skid0 || 0.14 },
  ...
  dash: { key: 'transDash', cells: 6, k: HERO_DELIVERY_K, t: p => p.dashT, t0: p => p.dash0 || 0.16 },
  ...
  hurt: { key: 'hzdHurt', cells: 4, k: HERO_DELIVERY_K, t: p => p.hurtPoseT, t0: () => 0.3 },
  ...
  heal: { key: 'hzdHeal', cells: 5, k: HERO_DELIVERY_K, t: p => 0.85 - p.healT, t0: () => 0.85 },
  ...
  guard: { key: 'hzdGuard', cells: 1, k: HERO_DELIVERY_K, t: p => p.guardT, t0: () => 1 },
  //
...
let HERO_TAKEOFF = { key: 'transTakeoff', cells: 4, k: HERO_DELIVERY_K };
...
let HERO_AIR_STRIP = { key: 'transAir', cells: 8, k: HERO_DELIVERY_K, up: 770, down: 700 };
...
let HERO_DEATH_STRIP = { key: 'hzdDeath', cells: 6, k: HERO_DELIVERY_K, total: 1.8, hold: 1.2 };
...
let HERO_FIDGET = { key: 'heroFidget', cells: 8, k: HERO_DELIVERY_K, fps: 6.5, intro: 0 };
...
const SWING_STRIP = {
  ...
  burstLegacy: {key:'swingClawChargeAlt',cells:12,k:HERO_DELIVERY_K,order:[6,7,8,9,10,11],fx:HERO_BURST_FX},
  burst:    { key: 'swingClawCharge', cells: 12, k: HERO_DELIVERY_K, order:[6,7,8,9,10,11], fx: HERO_BURST_FX },
};
...
// Weapon bodies use the same attack clock and retain the unarmed fallback until loaded.
const HERO_SINGLE_FX = {key:'swingSingleFx',cells:6,release:{from:0,to:5},dx:42,dy:0,size:64,alpha:.68};
// [effect cell, x, y offset from torso centre, size] for each body pose.
const HERO_SINGLE_FX_POSES = [null,[0,30,-8,48],[1,0,-28,58],
  [2,45,4,60],[3,43,-4,58],[1,5,-25,54],[4,46,0,65],
  [3,45,12,58],[5,42,9,48],null];
const HERO_DUAL_FX={key:'swingDualFx',cells:8,release:{from:0,to:7},dx:35,dy:0,size:64,alpha:.7};
const HERO_DUAL_FX_POSES=[null,null,[1,38,-5,58],[2,38,2,58],[3,5,-5,70],[5,48,2,54],[6,38,8,48],null];
const HERO_SINGLE_NORMAL_FX_POSES=[null,null,[3,35,10,60],[2,10,-12,64],[1,10,-24,62],[4,40,2,68],[5,35,9,48],null];
SWING_STRIP.air.fx={...HERO_SINGLE_FX,release:{from:1,to:2},dx:22,dy:0,size:62};
SWING_STRIP.down.fx={...HERO_SINGLE_FX,release:{from:1,to:2},dx:0,dy:16,size:60,rotation:Math.PI/2};
const HERO_WEAPON_CHARGES={single:{key:'swingSingleCharge',cells:10,k:HERO_DELIVERY_K,order:[3,4,5,6,7,8,9],fx:HERO_SINGLE_FX,fxPoses:HERO_SINGLE_FX_POSES},dual:{key:'swingDualCharge',cells:10,k:HERO_DELIVERY_K,order:[3,4,5,6,7,8,9],fx:HERO_DUAL_FX,
  fxPoses:[null,[0,0,0,50],[3,0,0,64],[3,0,-5,74],[3,0,-5,78],[3,0,-5,82],[4,12,-5,80],[5,12,0,84],[7,15,5,60],null]}};
const HERO_JOINED_FX_POSES=[null,[3,0,-4,62],[2,34,10,62],[1,33,-12,65],[3,0,-34,65],[5,49,0,58],[6,40,5,50],null];
const HERO_WEAPON_SWINGS = {
  joined:[
    {key:'swingJoined',cells:8,k:HERO_DELIVERY_K*1.2,fx:HERO_DUAL_FX,fxPoses:HERO_JOINED_FX_POSES,order:[0,1,2,6,7]},
    {key:'swingJoined',cells:8,k:HERO_DELIVERY_K*1.2,fx:HERO_DUAL_FX,fxPoses:HERO_JOINED_FX_POSES,order:[0,2,3,6,7]},
    {key:'swingJoined',cells:8,k:HERO_DELIVERY_K*1.2,fx:HERO_DUAL_FX,fxPoses:HERO_JOINED_FX_POSES,order:[0,3,4,5,6,7]},
  ],
  dual:[
    {key:'swingDual',cells:8,k:HERO_DELIVERY_K,fx:HERO_DUAL_FX,fxPoses:HERO_DUAL_FX_POSES,order:[0,1,2,3,6,7]},
    {key:'swingDual',cells:8,k:HERO_DELIVERY_K,fx:HERO_DUAL_FX,fxPoses:HERO_DUAL_FX_POSES,order:[0,3,2,5,6,7]},
    {key:'swingDual',cells:8,k:HERO_DELIVERY_K,fx:HERO_DUAL_FX,fxPoses:HERO_DUAL_FX_POSES,order:[0,1,4,5,6,7]},
  ],
  single: [
    {key:'swingSingle',cells:8,k:HERO_DELIVERY_K,fx:HERO_SINGLE_FX,fxPoses:HERO_SINGLE_NORMAL_FX_POSES,order:[0,1,2,6,7]},
    {key:'swingSingle',cells:8,k:HERO_DELIVERY_K,fx:HERO_SINGLE_FX,fxPoses:HERO_SINGLE_NORMAL_FX_POSES,order:[0,2,3,5,6,7]},
    {key:'swingSingle',cells:8,k:HERO_DELIVERY_K,fx:HERO_SINGLE_FX,fxPoses:HERO_SINGLE_NORMAL_FX_POSES,order:[0,1,3,4,5,6,7]},
  ],
};
function heroWeaponSwing(sv) {
  if (!sv) return null;
  if (sv.charged) return HERO_WEAPON_CHARGES[sv.weaponMode] || null;
  if (sv.air) return null;
  const strips=HERO_WEAPON_SWINGS[sv.weaponMode];
  return strips && strips[Math.max(0,Math.min(2,sv.combo|0))];
}
function warmHeroWeaponArt(mode) {
...
const HERO_STEP_WALK = 29.3, HERO_STEP_RUN = 38;
...
let HERO_GAIT = {
  walk: { key: 'gaitWalk', cells: 8, k: HERO_DELIVERY_K },
  run:  { key: 'gaitRun',  cells: 16, k: HERO_DELIVERY_K, from: 0, to: 15 },
};
...
let HERO_IDLE = { key: 'hzdIdle', cells: 8, k: HERO_DELIVERY_K, fps: 6.5 };
...
const HERO_EYE = {
  idle:        { lx: 0.592, ly: 0.561, rx: 0.653, ry: 0.565, ew: 0.020, eh: 0.055 },
  walk_a:      { lx: 0.577, ly: 0.511, rx: 0.647, ry: 0.515, ew: 0.023, eh: 0.059 },
  walk_b:      { lx: 0.579, ly: 0.511, rx: 0.647, ry: 0.516, ew: 0.022, eh: 0.058 },
  run_a:       { lx: 0.586, ly: 0.557, rx: 0.653, ry: 0.561, ew: 0.022, eh: 0.056 },
  run_b:       { lx: 0.587, ly: 0.564, rx: 0.654, ry: 0.567, ew: 0.023, eh: 0.056 },
  rise:        { lx: 0.743, ly: 0.272, rx: 0.812, ry: 0.265, ew: 0.027, eh: 0.058 },
  apex:        { lx: 0.643, ly: 0.411, rx: 0.710, ry: 0.422, ew: 0.027, eh: 0.056 },
  fall:        { lx: 0.625, ly: 0.550, rx: 0.678, ry: 0.573, ew: 0.037, eh: 0.050 },
  land:        { lx: 0.684, ly: 0.653, rx: 0.739, ry: 0.660, ew: 0.025, eh: 0.056 },
  dash:        { lx: 0.884, ly: 0.463, rx: 0.926, ry: 0.472, ew: 0.025, eh: 0.050 },
  skid:        { lx: 0.507, ly: 0.537, rx: 0.568, ry: 0.541, ew: 0.022, eh: 0.058 },
  wall_cling:  { lx: 0.577, ly: 0.363, rx: 0.632, ry: 0.380, ew: 0.027, eh: 0.044 },   // BY HAND (detector scored 6.28)
  djump_jet:   { lx: 0.691, ly: 0.307, rx: 0.757, ry: 0.311, ew: 0.023, eh: 0.058 },
  claw_1:      { lx: 0.691, ly: 0.588, rx: 0.740, ry: 0.597, ew: 0.023, eh: 0.050 },
  claw_2:      { lx: 0.642, ly: 0.553, rx: 0.699, ry: 0.559, ew: 0.019, eh: 0.056 },
  finisher:    { lx: 0.685, ly: 0.495, rx: 0.750, ry: 0.499, ew: 0.022, eh: 0.058 },
  charge:      { lx: 0.661, ly: 0.623, rx: 0.720, ry: 0.631, ew: 0.027, eh: 0.053 },
  burst:       { lx: 0.579, ly: 0.564, rx: 0.644, ry: 0.571, ew: 0.023, eh: 0.058 },
  hurt:        { lx: 0.629, ly: 0.434, rx: 0.687, ry: 0.451, ew: 0.031, eh: 0.056 },
  heal:        { lx: 0.601, ly: 0.515, rx: 0.662, ry: 0.521, ew: 0.022, eh: 0.056 },
  song:        { lx: 0.592, ly: 0.517, rx: 0.654, ry: 0.518, ew: 0.020, eh: 0.056 },
  slump:       { lx: 0.637, ly: 0.697, rx: 0.707, ry: 0.720, ew: 0.047, eh: 0.041 },
  walk_c:      { lx: 0.581, ly: 0.513, rx: 0.649, ry: 0.517, ew: 0.023, eh: 0.058 },
  run_c:       { lx: 0.589, ly: 0.567, rx: 0.655, ry: 0.570, ew: 0.022, eh: 0.056 },
};
...
class Player {
  ...
  update(dt) {
    ...
    this.throwPoseT = Math.max(0,(this.throwPoseT||0)-dt);
    this.catchPoseT = Math.max(0,(this.catchPoseT||0)-dt);
    if (this.slowT > 0) {
      this.slowT -= dt;
    ...
    if (this.dashT > 0) {
      ...
      if (inP('DASH') && hasMod('dash') && this.dashCD <= 0) {
        ...
        if (typeof padRumble === 'function') padRumble(0.10, 0.22, 45);
      }
    }
    ...
    if (this.atkBuf > 0 && this.atkCD <= 0 && this.swirlT <= 0) {
      ...
      if (typeof padRumble === 'function') padRumble(0.04, 0.10, 22);
      }
    }
    ...
    if (inD('HEAL') && this.on && this.dashT <= 0 && this.volts >= this.healCost() && this.cores < this.maxCores()
      ...
      if (this.healT >= 0.85) {
        ...
        if (typeof padRumble === 'function') padRumble(0.12, 0.25, 90);
  ...
  swirlPass() {
    ...
    if (hit) { G.hitStop = Math.max(G.hitStop, 0.035); sfx('hit');
      if (typeof padRumble === 'function') padRumble(0.22, 0.38, 65);
    }
    this.swirlHits += hit;
  ...
  throwJoined() {
    ...
    this.vx -= dir * 90;
    this.throwPoseT = 0.18; this.catchPoseT = 0;
    this.healT = 0;
    sfx('crystalJoin');
  ...
  drawRoboSwing(c) {
    this._authoredSwingFx = null;
    const sv = this.swingVis;
    if (!sv) return false;
    const weapon = heroWeaponSwing(sv);
    const ready = weapon && typeof MEDIA_RAW !== 'undefined' && MEDIA_RAW[weapon.key] && MEDIA_RAW[weapon.key].naturalWidth;
    if (weapon && !ready && typeof mediaFetch === 'function') mediaFetch(weapon.key,1);
    if (sv.swirl && !ready) return false;
    const S = ready ? weapon : sv.chargeArtAlt ? SWING_STRIP.burstLegacy : SWING_STRIP[heroSwingState(sv)];
    ...
    let cell = S.order ? S.order[Math.min(S.order.length-1,Math.floor(p*S.order.length))] : S.pingpong
      ? (p < 0.5 ? Math.floor(p * 2 * S.cells) : Math.floor((1 - p) * 2 * S.cells))
      : Math.floor(p * S.cells);
    if(S===SWING_STRIP.down && this.on)cell=4+Math.min(1,Math.floor(p*2));
  ...
  drawSwingFx(c, FX, range, p, cx, cy, size) {
    ...
    if(FX.rotation){c.translate(cx,cy);c.rotate(FX.rotation);cx=0;cy=0;}
  ...
  drawRoboRecharge(c) {
    const rc=G.recharge;
    if(!rc || this.dead)return false;
    const phase=rc.phase==='dock'?Math.max(0,Math.min(.999,1-rc.dockT/rc.dock0)):
      Math.max(0,Math.min(.999,1-rc.t/rc.dur));
    const frame=rc.phase==='dock'?Math.floor(phase*4):4+Math.floor(phase*4);
    return drawStripCell(c,'heroRecharge',frame,8,0,HERO_FLOOR,HERO_DH*HERO_DELIVERY_K,false);
  }
  drawRoboCharge(c) {
    if (this.dead || this.hurtPoseT > 0 || this.dashT > 0 || this.swingVis ||
        !this.on || Math.abs(this.vx)>34 || this.chargeT <= .08 || G.boomer) return false;
    const mode=weaponMode(G.save);
    if(mode==='joined')return false;
    const S=HERO_WEAPON_CHARGES[mode] || (this._chargeArtCount>0 && this._chargeArtCount%2===0 ? SWING_STRIP.burstLegacy : SWING_STRIP.burst);
    const count=mode==='single'||mode==='dual'?3:6;
    const frame=Math.min(count-1,Math.floor(this.chargeT/.12));
    return drawStripCell(c,S.key,frame,S.cells,0,HERO_FLOOR,HERO_DH*S.k,false);
  }
  drawRoboJoinedThrow(c) {
    if (this.dead || this.hurtPoseT > 0 || this.dashT > 0 || this.swingVis) return false;
    let frame = -1;
    if (this.throwPoseT > 0 && G.boomer) frame=3; // actual released weapon means empty paws
    else if (this.catchPoseT > 0 && !G.boomer) frame=this.catchPoseT>.09?4:5;
    else if (!G.boomer && this.on && Math.abs(this.vx)<=34 && this.chargeT>.08 && weaponMode(G.save)==='joined')
      frame=Math.min(2,Math.floor(this.chargeT/.2));
    if(frame<0)return false;
    return drawStripCell(c,'heroJoinedThrow',frame,6,0,HERO_FLOOR,HERO_DH*HERO_DELIVERY_K,false);
  }
  drawRoboPlate(c, run) {
    c.save();
    ...
    if (this.drawRoboRecharge(c)) { G.heroDrawn=G.lastStrip; return true; }
    if (this.drawRoboJoinedThrow(c)) { G.heroDrawn=G.lastStrip; return true; }
    if (this.drawRoboCharge(c)) { G.heroDrawn=G.lastStrip; return true; }
    ...
    // Critical motion never falls through to retired pose sheets. A missing
    // essential gait sheet also actively re-requests the asset rather than
    // waiting on the passive lazy fetch.
    if (/^(walk_|run_)/.test(st) || ['rise','apex','fall','land','dash','skid','wall_cling'].includes(st)) {
      if (/^(walk_|run_)/.test(st) && typeof requestHeroArt === 'function') requestHeroArt();
      if (typeof G !== 'undefined') G.heroDrawn = 'loading:' + st;
      return true;
    }
    ...
    const dh = HERO_DH * (HERO_POSE_K[st] || HERO_DELIVERY_K), dw = dh * (cw / ch);
...
function drawTurretLock(c, e, cx) {
  const cy = e.y + e.h / 2;
  // across the lock it was ACTUALLY given: a conduit gun's is longer (see
  // CONDUIT_TRAIN_LOCK), and a beam that finished brightening early would be
  // reporting a shot that has not been fired yet
...
// ===========================================================================
// KINGDOM A — THE SCRAP MEADOWS ARM THEIR OWN MACHINES.
//
// One idea, worn three ways: the meadow is made of scrap, and a machine that
// has been here long enough learns to PICK THE SCRAP UP. Nothing here is a new
// body, a new sheet or a new state machine — it is the roster she already knows
// reaching down and using the floor she has been walking on since the first
// minute of the game. That is what a returning player is supposed to read:
// not "a new enemy", but "these have been busy".
//
// THE CONSTRAINT NO OTHER KINGDOM HAS. foeLevel('A') is 1 on a fresh save,
// which is ZERO points, which buys none of this — so the tutorial, the waking
// floor and the first hour are byte-for-byte the game they have always been.
// The run's own progress is what raises the meadow to level 2-3, so these
// appear on the walk BACK through it and nowhere else. tests/scrapmoves.cjs
// measures both halves of that, and the fresh-save half is the important one.
//
// EVERY ONE OF THEM PAYS FOR ITS REACH WITH A LONGER WARNING. The law above
// TRAITS is not relaxed by a level: an enemy is never harder because it warned
// you less. A crawler that drags a plate has to pick the plate up first, and
// picking it up takes TELL_SWIPE where the bare lunge took TELL_FAST — the tell
// gets 43% LONGER, in exchange for the reach. Same for the hopper's rake. The
// blob's bloom goes further still and paints the two tiles it is about to throw
// at, because a hazard that lands where she is not standing has to be shown or
// it is not a read at all.
FOE_MOVES['crawler@A'] = [{ id: 'scrapdrag', cost: 1 }];
FOE_MOVES['hopper@A'] = [{ id: 'scrapkick', cost: 1 }];
FOE_MOVES['blob@A'] = [{ id: 'rustbloom', cost: 1 }];
// The bloom's sag, against the plain drip's 0.34: nearly twice the warning for
// a throw that leaves the tile under it and takes the two beside it instead.
const BLOB_BLOOM = 0.62;
// how far a bloom throws, how far a dragged plate reaches past the nose, and
// how far the hopper's rake washes along the floor — all in pixels, all read
// off the body so a trait that changes its size does not change its reach
const BLOOM_THROW = 30, DRAG_REACH = 16, KICK_REACH = 96;
...
// ===========================================================================
// KINGDOM X — THE CRYSTAL CACHE BENDS THE ANSWER (owner, 2026-09-18: "improve
// enemy level and skills point with more moves that need to be created for
// them with every kingdom"). The sixth and last kingdom to file its rows.
//
// `@X`-SCOPED AND NOTHING ELSE, per the registry note above FOE_MOVES: the
// unscoped rows are the integrator's and five other sessions own their own
// letters. Zone X is the Cache AND the crystal cave that shares its seam
// (CV1..CV3 carry zone 'X' in js/world.js), so these rows are what the whole
// quarry builds — which is also why the roster here is exactly three kinds.
//
// THE ROSTER IS SMALL AND THAT IS THE SHAPE OF THE KINGDOM. Measured off
// world.js rather than assumed: the only enemy kinds standing anywhere in
// zone X are CRAWLER, HOPPER and BAT. No kiln, no turret, no guard, no blob —
// so there is nothing here to hide a weak idea behind. Three machines, three
// moves, one each.
//
// THE ONE SENTENCE ALL THREE SPEAK: THE CACHE REFRACTS THE ANSWER. Every
// other kingdom buys reach, cadence, ground or biology; down here the machine
// is grown through with seam crystal and what bends is the LINE the attack
// travels. So each move is aimed squarely at the ANSWER THE PLAYER ALREADY
// LEARNED for that machine, and at nothing else:
//
//   facet   (crawler, 2) YOU LEARNED TO STEP BEHIND IT. The facet grown along
//                        its spine splits the lunge, and the frame it commits
//                        a lance sweeps the floor it left — behind it. The
//                        forward lunge, the reach and the winded 0.55 s are
//                        all untouched; what changes is that the free side
//                        stops being free.
//   refract (hopper,  3) YOU LEARNED TO READ THE ARC. It leaves on the honest
//                        line and turns ONCE, at the top, onto a spot the
//                        floor has been painted with since the crouch began.
//                        The only attack in the game whose DESTINATION is
//                        told instead of inferred — and the read moves from
//                        the machine to the ground.
//   glint   (bat,     1) YOU LEARNED TO CHASE IT DOWN. The dive sheds
//                        splinters that hang in the air where it passed, so
//                        the lane behind it is briefly not a corridor. The
//                        game's only AIRBORNE hazard: the blob's pool taught
//                        the floor going hostile, this teaches the air.
//
// THE PRICES ARE THE KINGDOM'S DEPTH. FOE_ZONE_LV.X is 5 — tied with the Nest
// for the highest floor in the game — so a Cache machine lands with 4 points
// and reaches 6. That is what pays for a cost-3: `refract` needs four points,
// which is level 5, which is this kingdom's floor and the Nest's and nobody
// else's. At 1 the bat's splinters are on every bat down here from the first
// walk, because a kingdom's own question should be asked at its door.
//
// AND EVERY ONE OF THEM WARNS FIRST — the law above TRAITS is not relaxed by
// depth, and this is the deepest room in the game, where it matters most.
// Two of the three LENGTHEN a tell (the crawler's coil and the bat's shiver
// both go TELL_FAST -> TELL_SWIPE, 43% more warning) and the third paints its
// landing spot on the floor for the whole crouch AND the whole flight, which
// is over a second of notice for a machine that used to tell you nothing
// about where it would come down. Not one of them shortens anything.
FOE_MOVES['bat@X'] = [{ id: 'glint', cost: 1 }];
FOE_MOVES['crawler@X'] = [{ id: 'facet', cost: 2 }];
FOE_MOVES['hopper@X'] = [{ id: 'refract', cost: 3 }];
// FACET. The rear lance reaches a little further than the body is wide and
// lives for a fifth of a second — long enough to be a place, short enough
// that it is a sweep and not a wall. It is deliberately SHORTER-lived than
// the winded window it shares a beat with: the punish is still there, it just
// has to be taken from the front or from above now.
const FACET_REACH = 34, FACET_LIVE = 0.22;
// REFRACT. How far off the honest arc the bend lands, the ceiling on the
// speed it may turn onto (so a bend is a turn and never a teleport), and
// where in the flight it turns — 0.28 s is the apex of the hopper's own
// -560 / 2000 arc, so it bends at the top where the change is most visible.
// REFRACT_FLIGHT is the hopper's own hang time, 2 * 560 / 2000, and REFRACT_OFF
// is measured off the spot THAT arc would reach — not off her feet. Measuring
// it off the player was the version that did not work: from a take-off far
// enough away the bend could not cover the gap and the machine landed short of
// its own paint, which is a telegraph that lies.
//
// AND REFRACT_TURN IS A LIMIT ON THE CHANGE, NOT ON THE SPEED. That was the
// second half of the same bug: a ceiling on vx is a ceiling the honest arc has
// already spent most of, so a cap that reads as generous in the abstract left
// nothing to turn WITH. Measured off the arc the change is always exactly
// REFRACT_OFF / (FLIGHT - APEX) = 221 px/s whatever the machine's speed, so
// 300 is a turn every Cache hopper can make and no hopper can exceed.
const REFRACT_OFF = 62, REFRACT_TURN = 300, REFRACT_APEX = 0.28, REFRACT_FLIGHT = 0.56;
// GLINT. Each splinter hangs armed and HARMLESS for GLINT_ARM before it can
// touch anything — longer than TELL_FAST, and it is drawn amber for all of it
// — then bites for GLINT_LIVE. Six at most, one every GLINT_GAP, so a dive
// leaves a dotted line through the air rather than a wall of glass.
const GLINT_ARM = 0.42, GLINT_LIVE = 1.15, GLINT_GAP = 0.15, GLINT_R = 11, GLINT_MAX = 6;
// ---------------------------------------------------------------------------
// facet: the strip of floor BEHIND the machine, computed from where it is
// standing and which way it is facing. One function so the amber the tell
// draws and the lance that actually fires can never disagree — a telegraph
// that is computed twice is a telegraph that eventually lies. It tracks the
// dir through the coil on purpose: a clever crawler turns to keep its nose on
// her while it gathers, and the strip turning with it is visible, which is
// exactly how the crawler's facing has always been part of its warning.
function facetStrip(e) {
  const cx = e.x + e.w / 2, back = -(e.dir || 1);
  return { x: back > 0 ? cx + e.w * 0.4 : cx - e.w * 0.4 - FACET_REACH,
    y: e.y + 2, w: FACET_REACH, h: e.h - 4 };
}
// ...and the lance once it has fired, FROZEN where the tell promised it. The
// body is already moving away by then — it is mid-lunge — so a strip that
// rode along with it would be a different strip from the one she was shown.
function facetStep(e, dt) {
  if ((e.lanceT || 0) <= 0) return;
  e.lanceT -= dt;
  const lb = { x: e.lanceX, y: e.lanceY, w: FACET_REACH, h: e.lanceH };
  if (chance(dt * 34)) addPart(lb.x + rnd(0, FACET_REACH), lb.y + rnd(0, lb.h),
    rnd(-34, 34), rnd(-80, -10), 0.34, '#8fe8ff', 1.9, 320);
  if (player.dead || player.iT > 0) return;
  if (aabb(lb, player)) player.hurt(DF().edmg, lb.x + FACET_REACH / 2, 'crawler.facet');
}
// ---------------------------------------------------------------------------
// refract: choosing the spot the arc will bend onto. BOTH SIDES ARE TRIED and
// the one with floor under it wins — a mark painted over a pit is a promise
// the machine cannot keep, and a telegraph that lies is worse than no move at
// all. If neither side has ground, the leap is simply the leap it always was.
function refractMark(e, aim) {
  const rw = ((G.roomDef && G.roomDef.w) || 60) * TILE;
  const order = chance(0.5) ? [1, -1] : [-1, 1];
  for (const s of order) {
    const mx = clamp(aim + s * REFRACT_OFF, TILE * 1.5, rw - TILE * 1.5);
    let my = e.y + e.h;
    // THE FLOOR IS NOT THE TILE TOP. Terrain here is a HEIGHTFIELD (the NO
    // RIGHT ANGLES order: `groundColumnAt`), so a body resting on organic
    // ground stands ABOVE the tile it stands on — which means a probe that
    // starts two pixels under its own feet finds AIR and walks a whole tile
    // PAST the floor it is standing on. That painted the mark a tile into the
    // rock and the machine landed nowhere near it. Step in quarter-tiles, then
    // snap to the curve the way the Nest's creep rides it, so the paint sits
    // on the surface a body would actually come down on.
    for (let k = 0; k < 24; k++) {
      if (solidAt(Math.floor(mx / TILE), Math.floor((my + 2) / TILE))) {
        if (typeof groundColumnAt === 'function') {
          const gc = groundColumnAt(mx);
          if (gc && Math.abs(gc[0] - my) < TILE) my = gc[0];
        }
        return { x: mx, y: my };
      }
      my += TILE / 4;
...
// ---------------------------------------------------------------------------
// glint: the splinters a Cache bat sheds down its dive. They are kept on the
// BAT, like the breaker's waves and the Foundry's gobs, for the same two
// reasons — the room's hazard lists belong to the shared layer a kingdom may
// not reach into, and "kill the bat, the glass goes with it" is a better rule
// than a hazard that outlives whatever made it.
function glintStep(e, dt) {
  const m = e.motes;
  if (!m || !m.length) return;
  for (let i = m.length - 1; i >= 0; i--) {
    const g = m[i];
    if (g.arm > 0) {                       // ARMING — drawn, audible, harmless
      g.arm -= dt;
      if (g.arm <= 0) { g.arm = 0; sfx('shard'); }
      continue;
    }
    g.life -= dt;
    if (g.life <= 0) { m.splice(i, 1); continue; }
    if (chance(dt * 10)) addPart(g.x + rnd(-6, 6), g.y + rnd(-6, 6),
      rnd(-16, 16), rnd(-16, 16), 0.4, '#8fe8ff', 1.7, 30, true);
    if (player.dead || player.iT > 0) continue;
    if (Math.abs(player.x + player.w / 2 - g.x) < GLINT_R + player.w * 0.4
      && Math.abs(player.y + player.h / 2 - g.y) < GLINT_R + player.h * 0.4)
      player.hurt(DF().edmg, g.x, 'bat.glint');
  }
}
// KINGDOM B — THE DATA CONDUITS' MOVES (owner, 2026-09-18: "improve enemy level
// and skills point with more moves that need to be created for them with every
// kingdom").
//
// Every row here is `@B`-SCOPED AND NOTHING ELSE. The unscoped rows are the
// integrator's, and five kingdom sessions writing `flier:` would each silently
// overwrite the last — the scoped key is the whole reason this is safe to work
// on in parallel (see the note above FOE_MOVES).
//
// The three moves are the kingdom's own vocabulary made playable — SIGNAL,
// PACKET, BANDWIDTH — and each one asks a question the conduits already ask
// with their terrain, one machine at a time:
//
//   relay  (surge, 2)   THE BREAKER IS A REPEATER. Its floor wave no longer
//                       dies at the wall it reaches: it latches there and runs
//                       back down the rail. Behind the wave stops being
//                       automatically safe, which is the one place the plain
//                       breaker let her stand and stop thinking.
//   packet (flier, 1)   THE SURVEY LENS IS A COURIER. It holds station, draws
//                       the lane it is about to drop into, and delivers a
//                       charge packet to the floor. The air stops being the
//                       only thing the flier owns — and the tile she is
//                       standing on to hit it is the tile it is aiming at.
//   train  (turret, 2)  BANDWIDTH IS METERED. The gun declares a count and
//                       spends it: three shots at a fixed cadence whatever the
//                       range, after a lock half again as long as the ordinary
//                       one. Crossing the lane becomes counting, not guessing.
//
// THE PRICES ARE THE KINGDOM'S SHAPE. B's floor is level 2 (FOE_ZONE_LV), which
// is ONE point, so a conduit machine met on a fresh save carries the courier's
// packet and nothing else. The repeater and the metered train cost two and
// arrive only once the run has raised the level under them. A player's first
// walk through the conduits is the kingdom it always was plus one new question;
// a player who comes back having taken four powers meets all three.
//
// AND EVERY ONE OF THEM WARNS FIRST. The law above TRAITS is not relaxed by a
// move — an enemy is never harder because it warned you less — so: the repeater
// wears a second ring through its WHOLE charge (this one comes back is known
// before the first wave is on the floor) and then latches visibly, stationary
// and harmless, for CONDUIT_RELAY_HOLD at the wall before it turns; the courier
// holds TELL_SWIPE dead still with its drop lane drawn all the way to the floor
// it will hit; and the train's lock is LONGER than the plain gun's and draws a
// pip for every shot it intends to send. Not one of the three shortens a tell
// and two of them lengthen one. Difficulty is in what they do.
// ===========================================================================
FOE_MOVES['flier@B'] = [{ id: 'packet', cost: 1 }];
FOE_MOVES['surge@B'] = [{ id: 'relay', cost: 2 }];
FOE_MOVES['turret@B'] = [{ id: 'train', cost: 2 }];
// The repeater's latch at the wall: the return trip's entire telegraph, and the
// wave neither moves nor hurts for any of it. Sized against TELL_FAST — long
// enough to be read from across the room, short enough that baiting the vent
// and walking in behind it is still the right answer, just no longer a free one.
const CONDUIT_RELAY_HOLD = 0.3;
// The courier's packet: how hard it falls, how long the charge it spills stays
// live, how wide that charge reaches, and how many one lens may have in flight.
// The cap matters — three couriers in a gallery could otherwise carpet a floor
// they were meant to punctuate.
const CONDUIT_PACK_G = 1500, CONDUIT_PACK_LIVE = 1.0, CONDUIT_PACK_R = 26, CONDUIT_PACK_MAX = 3;
// The metered train: the declared count, the gap between packets, and the lock
// that pays for declaring it — 0.8 s against the plain gun's 0.55.
const CONDUIT_TRAIN_N = 3, CONDUIT_TRAIN_GAP = 0.2, CONDUIT_TRAIN_LOCK = 0.8;
// ONE SHOT FROM A TURRET, wherever it is fired from. Extracted rather than
// copied so the metered train fires exactly the gun the plain turret fires: a
// train that drifted from the single shot would be a second weapon wearing the
// first one's name, and the drift would never show up in a screenshot.
function turretShot(e, cx, cy, px, py) {
  // a practised gunner shoots where you will be — unchanged, and the train
  // inherits it rather than restating it
  const aimX = leadX(px, e.iq * 0.42);
  const d = Math.hypot(aimX - cx, py - cy) || 1;
  G.projs.push(new Proj(cx, cy - 6, (aimX - cx) / d * 340, (py - cy) / d * 340, false, 1, 6, '#ff5c6c'));
  sfx('shoot');
  // the lead the bolt was given, handed back: the Archives' REDACT lays its bar
  // on the ground the shot is going to, and it has to be the SAME number or the
  // bar and the bolt stop agreeing
  return aimX;
}
// WHERE THE BREAKER'S WAVE ACTUALLY HAS A RAIL — and the reason this exists.
//
// The wave used to probe ONE fixed cell: the tile 4 px under the foot the wave
// was vented at. That is correct only on ground whose surface is exactly a tile
// top, and since the heightfield landed (groundColumnAt, the NO RIGHT ANGLES
// rule) no ground in this game is. A body stands on the CURVE, which sits above
// the square that holds it up — 15.6 px above it where the breaker stands in
// B2, 1.9 px in B1 — so the probe read the air over the floor, `railOn` came
// back false on the frame after the vent, and every wave the machine ever sent
// died where it was born. Measured in both of the breaker's own rooms before
// this line existed: 1 frame of life, 0 px of travel. The kingdom's signature
// attack was a sound and a puff of sparks.
//
// So it asks for the support the walkers already ask for (groundAhead), and the
// wave RIDES what it finds instead of carrying the height it was born at across
// a room that is never level. It will follow a step DOWN — a floor wave that
// stopped at a lip would be stranger than one that did not — and it still will
// not climb a ledge, which is the property the machine's read depends on: a
// full tile up is outside the band and returns null, so "get above it" stays
// the answer it has always been.
// RAIL_RISE is how much the rail may come UP under the wave in one step, and it
// is the whole of "it never climbs a ledge" expressed as a number. The wave
// covers about 4 px a frame, so a mound at 45 degrees hands it 4 px of rise —
// ten is generous for real terrain and far under the sixteen a hull lip offers.
const RAIL_RISE = 10, RAIL_DROP = TILE * 1.6;
function railSurfaceY(x, y) {
  if (typeof groundColumnAt === 'function') {
    const gc = groundColumnAt(x);
    // gc[0] < gc[1] is the proof that a solid tile is holding this surface up
    if (gc && gc[0] < gc[1] && y - gc[0] <= RAIL_RISE && gc[0] - y <= RAIL_DROP) return gc[0];
  }
  const tx = Math.floor(x / TILE);
  for (let ty = Math.floor(y / TILE); ty <= Math.floor((y + RAIL_DROP) / TILE); ty++) {
    // never a tile top ABOVE the wave: a lip is a wall to a thing crawling the
    // floor, and stepping up onto one is exactly the read this machine sells
    if (ty * TILE < y - 2) continue;
    const c = tileAt(tx, ty);
    if (c === '#' || c === 'B' || c === '=') return ty * TILE;
  }
  return null;
}
// Where the courier's packet will land, so the lane it draws ENDS ON THE FLOOR
// it will actually hit rather than running off the bottom of the frame. A
// warning that says "somewhere below" is not a warning about a place.
function conduitFloorY(x, fromY) {
  const tx = Math.floor(x / TILE);
  let ty = Math.floor(fromY / TILE);
  const rows = (G.grid && G.grid.length) || 0;
  for (; ty < rows; ty++) if (solidAt(tx, ty)) return ty * TILE;
  return rows * TILE;
}
// THE DELIVERED PACKETS, on their own clock. Called at the TOP of the flier's
// update, above every branch, because the dive path has its own early `break`
// and a packet frozen in mid-air through a dive would be a hazard that stops
// obeying gravity exactly when she stops watching it.
function conduitDrops(e, dt) {
  const ds = e.drops; if (!ds || !ds.length) return;
  const rows = (G.grid && G.grid.length) || 0;
  for (let i = ds.length - 1; i >= 0; i--) {
    const d = ds[i];
    d.ph += dt * 12;
    if (d.live > 0) {
      // SPILLED. The floor here is hostile, grounded-only — the same answer the
      // breaker's wave takes, because this is the same kingdom asking the same
      // question from the other side: the surge owns the floor, and the courier
      // is what puts a piece of the floor under someone who was in the air.
      d.live -= dt;
      if (d.live <= 0) { ds.splice(i, 1); continue; }
      if (!player.dead && player.iT <= 0 && player.on
          && Math.abs(player.x + player.w / 2 - d.x) < CONDUIT_PACK_R
          && Math.abs(player.y + player.h - d.y) < 16)
        player.hurt(DF().edmg, d.x, 'conduit.packet');
      if (chance(dt * 20))
        addPart(d.x + rnd(-CONDUIT_PACK_R, CONDUIT_PACK_R), d.y - 2, rnd(-24, 24), rnd(-70, -20), 0.3, '#ff9a6a', 2, 240, true);
      continue;
    }
    d.vy += CONDUIT_PACK_G * dt; d.y += d.vy * dt;
    const tx = Math.floor(d.x / TILE), ty = Math.floor((d.y + 5) / TILE);
    if (ty >= rows) { ds.splice(i, 1); continue; }   // dropped through the world
    if (solidAt(tx, ty)) {
      d.y = ty * TILE; d.vy = 0; d.live = CONDUIT_PACK_LIVE;
      sfx('spikeup');
      burst(d.x, d.y - 4, 10, '#ff9a6a', 180, 0.4, 70, 2.2, true);
    }
  }
}
// EVERY CONDUIT MOVE'S WARNING, DRAWN IN WORLD SPACE.
//
// It has to be here and not in the per-kind body art, because the body art is
// behind early returns: an authored plate (drawPlateAnchored) or the eagle-mini
// flier renderer both `return` before the procedural drum or lens is reached,
// and a telegraph that disappears the day the sprite sheet finishes loading is
// exactly the failure the turret's own lock light already suffered once. Called
// once from Enemy.draw, above everything that can return.
function drawConduitFX(c, e, cx) {
  if (e.dead) return;
  if (e.kind === 'surge' && e.hasMove('relay')) {
    // THE REPEATER DECLARES ITSELF BEFORE IT VENTS: a second ring and a pair of
    // out-and-back chevrons through the whole charge, so "this one comes back"
    // is known while there is still time to choose a side of the room — not
    // discovered when it returns.
    if ((e.crouchT || 0) > 0) {
      const k = 1 - clamp(e.crouchT / TELL_SWIPE, 0, 1);
      const ry = e.y + e.h * 0.4;
      c.save();
      c.globalAlpha = 0.3 + Math.sin(e.anim * 22) * 0.18;
      c.strokeStyle = TELL_COL; c.lineWidth = 1.6;
      for (const r of [16 + k * 9, 27 + k * 13]) { c.beginPath(); c.arc(cx, ry, r, 0, 7); c.stroke(); }
      c.globalAlpha = 0.75; c.fillStyle = TELL_COL;
      for (const s of [-1, 1]) {
        const ax = cx + s * (30 + k * 10);
        c.beginPath(); c.moveTo(ax + s * 5.5, ry); c.lineTo(ax, ry - 4); c.lineTo(ax, ry + 4); c.closePath(); c.fill();
        c.beginPath(); c.moveTo(ax - s * 4, ry); c.lineTo(ax + s * 1.5, ry - 4); c.lineTo(ax + s * 1.5, ry + 4); c.closePath(); c.fill();
      }
      c.restore(); c.globalAlpha = 1;
    }
    // ...and it LATCHES at the wall before it turns.
    if (e.waves) for (const w of e.waves) {
      if (!(w.hold > 0)) continue;
      const k = 1 - clamp(w.hold / CONDUIT_RELAY_HOLD, 0, 1);
      c.save(); c.globalCompositeOperation = 'lighter';
      c.globalAlpha = 0.45 + Math.sin(e.anim * 40) * 0.3;
      c.strokeStyle = TELL_COL; c.lineWidth = 2.4; c.lineCap = 'round';
      c.beginPath(); c.moveTo(w.x, w.y); c.lineTo(w.x, w.y - 16 - k * 10); c.stroke();
      c.globalAlpha = 0.4;
      c.beginPath(); c.arc(w.x, w.y - 8, 6 + k * 13, 0, 7); c.stroke();
      c.globalAlpha = 0.9; c.fillStyle = TELL_COL;
      const back = -w.dir;                       // where it is about to run
      c.beginPath(); c.moveTo(w.x + back * 13, w.y - 8);
      c.lineTo(w.x + back * 4, w.y - 13); c.lineTo(w.x + back * 4, w.y - 3); c.closePath(); c.fill();
      c.restore(); c.globalAlpha = 1;
    }
  }
  if (e.kind === 'flier' && e.hasMove('packet')) {
    // THE LANE, DRAWN BEFORE THE DROP. The warning says WHERE as well as WHEN,
    // which is the only thing that makes a drop from above a decision instead
    // of a tax — the same reason the turret's lock grew an aim line.
    if ((e.packetT || 0) > 0) {
      const k = 1 - clamp(e.packetT / TELL_SWIPE, 0, 1);
      const by = e.y + e.h, fy = conduitFloorY(cx, by);
      c.save();
      c.globalAlpha = 0.16 + k * 0.42;
      c.strokeStyle = TELL_COL; c.lineWidth = 1.5 + k * 1.5;
      c.setLineDash([7, 6]); c.lineDashOffset = -performance.now() / 40;
      c.beginPath(); c.moveTo(cx, by); c.lineTo(cx, fy); c.stroke();
      c.setLineDash([]);
      c.globalAlpha = 0.22 + k * 0.45;
      c.beginPath(); c.ellipse(cx, fy - 2, CONDUIT_PACK_R * (0.35 + k * 0.65), 5, 0, 0, 7); c.stroke();
      c.globalAlpha = 0.55 + Math.sin(e.anim * (18 + k * 30)) * 0.4;   // the belly lamp, quickening
      c.fillStyle = TELL_COL; c.shadowColor = TELL_COL; c.shadowBlur = 12;
      c.beginPath(); c.arc(cx, by - 2, 2.2 + k * 2.4, 0, 7); c.fill();
      c.shadowBlur = 0; c.restore(); c.globalAlpha = 1;
    }
    if (e.drops) for (const d of e.drops) {
      c.save();
      if (d.live > 0) {
        // the spilled charge wears the breaker's own red: in this game that hue
        // is reserved for the floor going hostile, and this IS the floor going
        // hostile. Ground-anchored, so G.artProbe switches it off with the rest.
        if (!G.artProbe) {
          const a = clamp(d.live / 0.35, 0, 1) * 0.55;
          c.globalCompositeOperation = 'lighter'; c.globalAlpha = a;
          c.strokeStyle = '#ff5f6d'; c.lineWidth = 2; c.lineCap = 'round';
          for (let i = 0; i < 5; i++) {
            const ax = d.x + (i - 2) * (CONDUIT_PACK_R / 2.6);
            const ah = 5 + Math.abs(Math.sin(d.ph + i * 1.3)) * 9;
            c.beginPath(); c.moveTo(ax - 4, d.y); c.quadraticCurveTo(ax, d.y - ah, ax + 4, d.y); c.stroke();
          }
          c.globalAlpha = a * 0.5;
          c.beginPath(); c.ellipse(d.x, d.y - 1, CONDUIT_PACK_R, 5, 0, 0, 7); c.stroke();
        }
      } else {
        // the packet itself, falling: a braced cube with its own lamp, so what
        // is coming down reads as a THING and not as a particle
        c.globalAlpha = 0.95;
        c.fillStyle = '#1d2733'; c.strokeStyle = '#ff8a5a'; c.lineWidth = 1.4;
        c.beginPath(); c.rect(d.x - 4, d.y - 4, 8, 8); c.fill(); c.stroke();
        c.globalAlpha = 0.5 + Math.sin(d.ph * 3) * 0.4;
        c.fillStyle = TELL_COL;
        c.beginPath(); c.arc(d.x, d.y, 1.8, 0, 7); c.fill();
      }
      c.restore(); c.globalAlpha = 1;
    }
  }
  if (e.kind === 'turret' && e.hasMove('train')) {
    // THE COUNT IS DECLARED. One pip per shot the gun intends to send: filling
    // as the (longer) lock runs, then going dark one at a time as the train
    // spends itself, so the number left is on screen while she is deciding
    // whether the gap is hers.
    const locking = (e.lockT || 0) > 0.02, firing = (e.trainT || 0) > 0;
    if (!locking && !firing) return;
    const k = locking ? 1 - clamp(e.lockT / (e.lock0 || CONDUIT_TRAIN_LOCK), 0, 1) : 1;
    c.save();
    for (let i = 0; i < CONDUIT_TRAIN_N; i++) {
      const pipx = cx + (i - (CONDUIT_TRAIN_N - 1) / 2) * 7.5, pipy = e.y - 13;
      const spent = firing && i < (e.burst | 0);
      const lit = !spent && (firing || k > i / CONDUIT_TRAIN_N);
      c.globalAlpha = spent ? 0.18 : lit ? 0.95 : 0.35;
      c.strokeStyle = TELL_COL; c.lineWidth = 1.2;
      c.beginPath(); c.arc(pipx, pipy, 2.4, 0, 7); c.stroke();
      if (lit) { c.fillStyle = TELL_COL; c.beginPath(); c.arc(pipx, pipy, 1.5, 0, 7); c.fill(); }
    }
    c.restore(); c.globalAlpha = 1;
  }
}
...
class Enemy {
  constructor(kind, x, y) {
    ...
    // KINGDOM A's three moves, declared here for the same reason:
    // `(this.crouchT -= dt)` on an undefined field is NaN and NaN is never > 0,
    // so a move whose fields are born on first use is a move that silently
    // never happens. dragT/rakeT are the tell lengths actually used (0 = this
    // machine did not buy the move), drag0/rake0 the 0..1 the draw hangs the
    // picked-up scrap on, plate/loaded whether it is carrying any, washT/washR
    // the hopper's floor ring, bloom/bloomN the blob's cycle count.
    this.dragT = 0; this.drag0 = 0; this.plate = 0;
    this.rakeT = 0; this.rake0 = 0; this.loaded = 0;
    this.washT = 0; this.washR = 0;
    this.bloom = false; this.bloomN = 0;
    // KINGDOM X's three, declared here for the same reason: a field born on
    // first use is a move that silently never happens. facetT/facet0 are the
    // crawler's rear-lance tell and the 0..1 its amber hangs on (0 = this
    // machine did not buy it), lanceT/lanceX/lanceY/lanceH the strip once it
    // is frozen and firing; bendT/bend0 the hopper's paint tell, markX/markY
    // the spot it painted and bendAt the flight time left before the arc
    // turns onto it; motes/glintT the bat's suspended splinters.
    this.facetT = 0; this.facet0 = 0; this.lanceT = 0;
    this.lanceX = 0; this.lanceY = 0; this.lanceH = 0;
    this.bendT = 0; this.bend0 = 0; this.bendAt = 0;
    this.markX = null; this.markY = null; this.arcX = null; this.bendDir = 0;
    this.motes = null; this.glintT = 0;
  ...
  // SCRAPDRAG lets go of its plate in three different places — the lunge
  // running out, a wall taking it, and the machine dying mid-lunge — and a
  // plate that survives any one of them is a hitbox with no picture on it.
  // One method, called from all three.
  shear() {
    if (!this.plate) return;
    this.plate = 0; this.drag0 = 0;
    const px0 = this.x + this.w / 2 + this.dir * (this.w * 0.6);
    burst(px0, this.y + this.h * 0.5, 7, '#b4702f', 190, 0.4, 620, 2.4, false);
  ...
  update(dt) {
    ...
    switch (this.kind) {
      ...
      case 'crawler': {
        ...
        // KINGDOM X — FACET (crawler@X, cost 2). The lance is ticked at the
        // top of the case, outside the branch chain, because it outlives the
        // frame it was thrown on: the body is already mid-lunge and moving
        // away from the floor the lance is sweeping. Nothing runs here for a
        // machine that did not buy the move, so a crawler in any other
        // kingdom is byte-for-byte the crawler it has always been.
        facetStep(this, dt);
        ...
        if (this.kind === 'guard' && (this.crouchT || 0) > 0) {   // THE TEAR
          ...
          // SCRAPDRAG, THE ACTIVE HALF: the plate it picked up rides ahead of
          // the nose, so the lunge has REACH the bare one never had. The
          // spacing she learned in the meadow's first fight no longer clears
          // it — which is the whole point of walking back through a kingdom.
          if (this.plate) {
            const pb = { x: this.dir > 0 ? this.x + this.w : this.x - DRAG_REACH,
                         y: this.y + 2, w: DRAG_REACH, h: this.h - 4 };
            if (!player.dead && aabb(pb, player))
              player.hurt(DF().edmg, cx, 'crawler.scrapdrag');
            if (chance(dt * 26))
          ...
          // SCRAPDRAG, THE WIND-UP: it is hooking a plate of meadow scrap up
          // off the floor, and drag0 is how far it has got. The draw hangs the
          // plate on this, so the warning is a THING rising in front of the
          // nose rather than a number — and the ring above it is already the
          // reserved amber.
          if (this.dragT > 0) this.drag0 = clamp(1 - this.coilT / this.dragT, 0, 1);
          // FACET'S WIND-UP: the seam crystal along its spine is filling, and
          // facet0 is how full. The draw hangs the rear strip's amber on this,
          // so the floor it is about to sweep brightens for the whole coil
          // rather than appearing on the frame it fires.
          if (this.facetT > 0) this.facet0 = clamp(1 - this.coilT / this.facetT, 0, 1);
          if (this.coilT <= 0) {
            ...
            if (this.dragT > 0) { this.plate = 1; this.drag0 = 1; this.dragT = 0; }
            // FACET'S ACTIVE HALF: the split goes off on the frame the body
            // commits, and the strip is FROZEN where the tell drew it. It is
            // a sweep of the floor it just left, not a second body — which is
            // why it is over in FACET_LIVE and why the winded window that
            // follows is exactly as long as it always was.
            if (this.facetT > 0) {
              const fs = facetStrip(this);
              this.lanceX = fs.x; this.lanceY = fs.y; this.lanceH = fs.h;
              this.lanceT = FACET_LIVE; this.facetT = 0; this.facet0 = 0;
              burst(fs.x + FACET_REACH / 2, fs.y + fs.h * 0.6, 12, '#8fe8ff', 220, 0.4, 260, 2.4, true);
              sfx('crack');
            }
            sfx('dash');
          }
          ...
          if (!player.dead && Math.abs(px - cx) < 160 && Math.abs(py - cy) < 70
            ...
            if (want !== this.dir) { this.dir = want; this.atkCD = 0.28 - this.iq * 0.14; }
              ...
              // THE TELL NEVER SHORTENS — and SCRAPDRAG makes it LONGER. The
              // reach is bought with warning, not stolen from it: hooking a
              // plate off the floor takes TELL_SWIPE where the bare lunge took
              // TELL_FAST, so the machine that hits further is also the one
              // that told you sooner. That trade is the only way a move is
              // allowed to be added to anything in this game.
              const drag = this.hasMove('scrapdrag');
              // ...and the Cache buys its rear lance with exactly the same
              // coin. A facet crawler coils for TELL_SWIPE where the bare one
              // coiled for TELL_FAST: the machine that owns the ground behind
              // it is also the one that told you sooner. (drag and facet can
              // never both be held — one row is crawler@A and the other
              // crawler@X — but the tell is written to take either.)
              const fac = this.hasMove('facet');
              this.coilT = (drag || fac) ? TELL_SWIPE : TELL_FAST;
              this.dragT = drag ? TELL_SWIPE : 0; this.drag0 = 0;
              this.facetT = fac ? TELL_SWIPE : 0; this.facet0 = 0;
              this.atkCD = rnd(2.2 - this.iq * 1.1, 3.4 - this.iq * 1.6);
              this.vx = 0; sfx((drag || fac) ? 'tellmid' : 'tell');
              // BOTH CHANNELS for the Cache too: the seam crystal rings under
              // the tell blip, so the crawler that owns its own back is told
              // apart from the one that does not without having to see it.
              if (fac) sfx('lumen');
        ...
        // a plate driven into a wall is a plate no longer: running it into the
        // scenery disarms the lunge, which makes the room part of the answer
        if (this.plate && (col.l || col.r)) this.shear();
        break;
      }
      ...
      case 'blob': {
        ...
        // RUSTBLOOM (kingdom A) changes the SHAPE of the drip, not its rate.
        // A meadow blob that has been sitting in the scrap long enough is full
        // of rust-water, and every third cycle it does not drip — it BLOOMS:
        // sags for nearly twice as long, then throws the load sideways and
        // leaves the tile under itself CLEAN. The read it adds is the one the
        // blob never asked before: standing next to it was always the answer,
        // and now it is the mistake. The long sag is the price of that, and the
        // draw paints the two tiles it is about to take while the sag runs, so
        // she is told where, not merely that.
        const tell = this.bloom ? BLOB_BLOOM : BLOB_TELL;
        this.drip0 = Math.max(0, Math.min(1, (tell - this.dripT) / tell));
        ...
        if (this.dripT <= 0 && this.on !== false) {
          ...
          if (this.bloom) {
            for (const s2 of [-1, 1])
              if (G.pools.length < 14)
                G.pools.push({ x: cx + s2 * BLOOM_THROW, y: this.y + this.h - 2, t: 4.2, t0: 4.2, r: 0 });
            burst(cx, this.y + this.h - 4, 10, '#b4702f', 210, 0.45, 520, 2.2, false);
            if (typeof sfx === 'function') sfx('lob');
          } else if (G.pools.length < 14) {
            G.pools.push({ x: cx, y: this.y + this.h - 2, t: 4.2, t0: 4.2, r: 0 });
          }
          // decided a whole cycle AHEAD, because a tell cannot lengthen for an
          // attack that has not been chosen yet
          this.bloomN = (this.bloomN | 0) + 1;
          this.bloom = this.hasMove('rustbloom') && this.bloomN % 3 === 0;
      ...
      case 'flier': {
        // KINGDOM B — THE COURIER (FOE_MOVES['flier@B'], 'packet'). Its packets
        // run on their own clock ABOVE every branch below, because the dive
        // path has its own early `break` and a packet frozen in mid-air through
        // a dive is a hazard that stops obeying gravity exactly when she stops
        // watching it. Both lines are inert on a flier that never bought one.
        if (this.drops && this.drops.length) conduitDrops(this, dt);
        ...
        if ((this.packetT || 0) > 0) {                        // the delivery's tell
          ...
          if (this.packetT <= 0) {
            ...
            if (this.drops.length < CONDUIT_PACK_MAX)
              this.drops.push({ x: cx, y: this.y + this.h, vy: 0, live: 0, ph: 0 });
            this.packetCD = rnd(3.4, 4.8);
          }
          moveEnt(this, dt);
          break;
        }
        if (this.diveT > 0) {
          ...
          if (this.hasMove('packet') && this.packetCD <= 0 && Math.abs(px - cx) < 70 && cy < py - 40) {
            this.packetT = TELL_SWIPE;     // longer than the dive's tell, never shortened by iq
            sfx('tell');
          } else if (Math.abs(px - cx) < 46 + this.iq * 26 && cy < py - 60 && (this.atkCD -= dt) <= 0) {
      ...
      case 'bat': {
        ...
        // KINGDOM X — GLINT (bat@X, cost 1). The splinters are ticked at the
        // top of the case because every branch below ends in a `break`, and a
        // hazard that only runs while the machine is diving would blink out
        // the moment it started climbing home. Nothing here runs for a bat
        // that did not buy the move.
        glintStep(this, dt);
        ...
        if (this.hang) {
          ...
          if (this.holdT > 0) {                     // the shiver — wings rustle
            ...
            // ...and a Cache bat shivers LONGER for it. The dive is about to
            // leave something behind it, so the dive is announced sooner:
            // TELL_SWIPE against the bare TELL_FAST, the same trade every
            // other kingdom's moves make.
            const gl = this.hasMove('glint');
            this.holdT = gl ? TELL_SWIPE : TELL_FAST;
            sfx(gl ? 'tellmid' : 'tell');
            if (gl) sfx('lumen');
        ...
        if (this.diveT > 0) {
          this.diveT -= dt;
          // GLINT'S ACTIVE HALF: seam crystal cracks off the brow as it dives
          // and HANGS where it fell out — the game's only airborne hazard.
          // Each splinter is dead for GLINT_ARM before it can touch anything,
          // amber for all of it, so the lane behind the bat is written in the
          // air a beat before it costs. Capped at GLINT_MAX: a dotted line
          // through the room, never a wall of glass.
          if (this.hasMove('glint')) {
            this.glintT = (this.glintT || 0) - dt;
            if (this.glintT <= 0) {
              this.glintT = GLINT_GAP;
              this.motes = this.motes || [];
              if (this.motes.length < GLINT_MAX)
      ...
      case 'turret': {
        ...
        // KINGDOM B — THE METERED TRAIN (FOE_MOVES['turret@B'], 'train'). Once
        // the first packet is away the rest of the train runs on its own gap,
        // not on the lock: the count was declared during a lock half again as
        // long as the plain gun's, and the pips have been on screen since.
        if ((this.trainT || 0) > 0) {
          this.trainT -= dt;
          if (this.trainT <= 0) {
        ...
        if ((this.lockT || 0) > 0) {
          ...
          if (this.lockT <= 0) {
            this.lockT = 0;
            // a practised gunner shoots where you will be (turretShot), the
            // lock is still the tell it always was and the beam still shows
            ...
            this.burst = (this.burst | 0) + 1;
            // ...and a conduit gun ignores the range entirely: the count it
            // showed you is the count you get, near or far, which is what makes
            // the gap countable instead of guessable.
            if (this.hasMove('train')) { this.trainT = CONDUIT_TRAIN_GAP; break; }
          ...
          // the train PAYS for declaring its count with a longer warning
          this.lockT = this.lock0 = this.hasMove('train') ? CONDUIT_TRAIN_LOCK : 0.55;
          sfx('ui');
        }
        break;
      }
      case 'hopper': {
        ...
        // SCRAPKICK, THE ACTIVE HALF (kingdom A): the heap it raked together
        // during the crouch goes out along the floor when it lands. It is a
        // GROUNDED hazard with a finite reach — jumped, or out-walked — which
        // is deliberately the same answer the Conduits' breaker wave wants, so
        // the meadow is where that answer is learned rather than where it is
        // sprung. It never climbs and it never follows: it is a ring on the
        // floor she was already told about.
        if ((this.washT || 0) > 0) {
          this.washT -= dt;
          this.washR = Math.min(KICK_REACH, (this.washR || 0) + 300 * dt);
          if (!player.dead && player.iT <= 0 && player.on
              && Math.abs(px - cx) < this.washR
              && Math.abs((player.y + player.h) - (this.y + this.h)) < 22)
            player.hurt(DF().edmg, cx, 'hopper.scrapkick');
          if (chance(dt * 34)) {
            const sd = chance(0.5) ? 1 : -1;
            addPart(cx + sd * this.washR, this.y + this.h - 2, sd * rnd(30, 120), rnd(-90, -20),
              0.34, '#b4702f', 2, 640);
          }
          if (this.washT <= 0) this.washR = 0;
        }
        // KINGDOM X — REFRACT, THE BEND (hopper@X, cost 3). The leap left the
        // floor on the honest line and turns ONCE, at the top of the arc,
        // onto the spot the floor has been painted with since the crouch
        // began. The turn is a turn and not a teleport: the speed it may bend
        // onto is capped, so the machine visibly swings across rather than
        // appearing somewhere. And it is bent at the APEX because that is the
        // slowest, highest, most readable moment of the whole jump.
        if ((this.bendAt || 0) > 0) {
          this.bendAt -= dt;
          if (this.bendAt <= 0) {
            this.bendAt = 0;
            // how long it has left in the air, solved rather than assumed, so
            // the bend arrives at the painted mark and not near it
            const d = (this.markY == null ? this.y + this.h : this.markY) - (this.y + this.h);
            const tf = d > 0
              ? (-this.vy + Math.sqrt(Math.max(0, this.vy * this.vy + 4000 * d))) / 2000
              : 0.22;
        ...
        if (col.d) {
          ...
          if (this.wasAir) {
            ...
            // ...and REFRACT lands ON the paint, which is the whole claim
            // the move makes. The mark goes out with the landing, so the
            // floor never carries a warning for a jump that is over.
            if (this.markX != null) {
              burst(cx, this.y + this.h - 2, 12, '#8fe8ff', 240, 0.45, 520, 2.4, false);
              sfx('shard');
              this.markX = null; this.markY = null; this.bendAt = 0;
            }
            // ...and the raked heap goes with it
            if (this.loaded) {
              this.loaded = 0; this.washT = 0.42; this.washR = 6;
              cam.shake = Math.max(cam.shake, 4);
              burst(cx, this.y + this.h - 2, 12, '#b4702f', 260, 0.45, 600, 2.4, false);
              if (typeof sfx === 'function') sfx('rock');
            }
          }
          this.wasAir = false;
          this.vx = 0;
          if (this.crouchT > 0) {
            this.crouchT -= dt;
            // the Foundry's move adds an ember shower for every frame of the
            // crouch; the meadow's adds a heap rising under the springs, whose
            // SIZE is the reach it is about to have. Two kingdoms, two
            // pictures, and neither of them shortens the crouch.
            if (this.hasMove('slagsplash')) emberTell(cx, this.y + this.h - 2, 40, dt);
            // the Cache's adds neither: the picture is on the FLOOR, a patch
            // of caustic light where it means to come down, and bend0 is how
            // far the crouch has got. She is told the destination before the
            // machine has left the ground.
            if (this.bendT > 0) this.bend0 = clamp(1 - this.crouchT / this.bendT, 0, 1);
            if (this.rakeT > 0) {
              this.rake0 = clamp(1 - this.crouchT / this.rakeT, 0, 1);
              if (chance(dt * 22))
                addPart(cx + rnd(-26, 26), this.y + this.h - 1, rnd(-18, 18), rnd(-40, -8),
                  0.3, '#b4702f', 1.6, 540);
            }
            if (this.crouchT > 0) break;
            if (this.rakeT > 0) { this.loaded = 1; this.rake0 = 1; this.rakeT = 0; }
          }
          this.t -= dt;
          ...
          if (this.t <= 0 && !player.dead && Math.abs(px - cx) < 380) {
            ...
            // saw coming.
            //
            // SCRAPKICK LENGTHENS IT, never the reverse: raking a heap of
            // meadow scrap under itself takes TELL_SWIPE where the bare gather
            // took TELL_FAST. Same trade the crawler's plate makes — reach
            // bought with warning.
            if (this.crouchT <= 0 && !this.gathered) {
              const rake = this.hasMove('scrapkick');
              // REFRACT'S WIND-UP, and it is the longest warning any of the
              // three Cache moves wears: the spot is CHOSEN AND PAINTED here,
              // at the top of a TELL_SWIPE crouch, and it stays lit through
              // the crouch AND the whole flight — over a second of notice for
              // a machine that used to tell you nothing at all about where it
              // would come down. If neither side of the honest arc has floor
              // under it the mark is not taken and the leap is the plain leap.
              const bendm = this.hasMove('refract');
              this.crouchT = (rake || bendm) ? TELL_SWIPE : TELL_FAST;
              this.rakeT = rake ? TELL_SWIPE : 0; this.rake0 = 0;
              this.bendT = 0; this.bend0 = 0; this.markX = null; this.markY = null;
              if (bendm) {
                // where the HONEST arc would put it down — same nose, same
                // speed, same hang time the leap below will use. The mark is
                // REFRACT_OFF to one side of THAT, not of her feet: the move
                // does not aim, it displaces, which is what keeps the answer
                // positional rather than a coin flip on which side it picked.
                this.bendDir = Math.sign(leadX(px, this.iq * 0.5) - cx) || this.dir || 1;
                this.arcX = cx + this.bendDir * this.spd * (1 + this.iq * 0.18) * REFRACT_FLIGHT;
                const mk = refractMark(this, this.arcX);
                if (mk) { this.markX = mk.x; this.markY = mk.y; this.bendT = TELL_SWIPE; }
              }
              this.gathered = true; sfx(rake ? 'tellmid' : 'tell');
            ...
            // a refract hopper leaves on the nose the paint was drawn for. It
            // is half a second since the spot was chosen and she may have
            // crossed it in the meantime; re-aiming here would send the body
            // one way and leave the promise lying on the floor the other.
            this.dir = this.markX != null
            ...
            this.bendT = 0;
            sfx('jump');
          } else if (this.markX != null && !this.wasAir) {
            // A MARK IS A PROMISE ABOUT A JUMP. If the crouch finished and the
            // leap never came — she walked out of the 380 band while it
            // gathered — the promise is dropped rather than left burning on a
            // floor nothing is coming to. A telegraph for an attack that is
            // not happening teaches the player to stop believing the floor,
            // which costs more than the move is worth.
            this.markX = null; this.markY = null; this.bendAt = 0;
          }
        }
        break;
      ...
      // gaps — unless it is a kingdom-B REPEATER, which latches at the wall and
      // comes back, see FOE_MOVES['surge@B'] — never climbs a ledge), or never
      // met at all (bait the vent and walk in behind it: 0.9 s of open vents
      // is a two-hit window that tempts three — the greed axis, per the combat
      // registry, and the repeater is what makes "behind it" a read of its own
      // rather than a free square). It is what
      ...
      case 'surge': {
        ...
        if (this.waves && this.waves.length) {
          for (let i = this.waves.length - 1; i >= 0; i--) {
            ...
            // KINGDOM B — THE REPEATER'S LATCH (FOE_MOVES['surge@B'], 'relay').
            // A relayed wave stops dead at the wall it reached and flares there,
            // stationary and HARMLESS, for the whole of CONDUIT_RELAY_HOLD
            // before it runs back. The return trip gets its own telegraph
            // rather than arriving out of a corner she had already cleared.
            if (w.hold > 0) {
              w.hold -= dt; w.ph += dt * 46;
              if (w.hold <= 0) { w.dir = -w.dir; w.life = w.life0 || w.life; w.x += w.dir * 6; }
              continue;
            }
            w.x += w.dir * w.spd * dt; w.life -= dt; w.ph += dt * 30;
            const wtx = Math.floor(w.x / TILE);
            const rail = railSurfaceY(w.x, w.y);           // the rail it rides
            const bty = Math.floor(((rail == null ? w.y : rail) - 10) / TILE);  // the air it needs
            const blocked = rail == null || solidAt(wtx, bty);
            // ...and only GEOMETRY sends it back. A wave that simply ran out of
            // life dies where it always died, so the move lengthens the reach of
            // a closed room and changes nothing at all in an open one.
            if (blocked && w.relay > 0) {
              w.relay--; w.x -= w.dir * w.spd * dt; w.hold = CONDUIT_RELAY_HOLD;
              sfx('tell'); continue;
            }
            if (w.life <= 0 || blocked) { this.waves.splice(i, 1); continue; }
            w.y = rail;                                    // ...and it rides it
        ...
        if (this.crouchT > 0) {            // the charge — fins up, amber on
          ...
          if (this.crouchT <= 0) {
            ...
            for (const s of [-1, 1])
              // life0 and relay are the repeater's: one return, at full reach
              this.waves.push({ x: cx + s * (this.w / 2 + 6), y: fy, dir: s, spd: this.spd,
                life, life0: life, ph: 0, hold: 0, relay: this.hasMove('relay') ? 1 : 0 });
  ...
  _die0(kx, ky) {
    if (this.dead) return;
    if (this.plate) this.shear();          // killed mid-lunge: the plate falls
    this.dead = true;
    const cx = this.x + this.w / 2, cy = this.y + this.h / 2;
    sfx('edie');
    player.gainVolts(8);
  ...
  draw(c) {
    ...
    // KINGDOM B'S WARNINGS, in world space and above every early return below —
    // an authored plate and the eagle-mini flier both return before the
    // procedural body is reached, and a telegraph that vanishes the day the
    // sprite sheet loads is the failure the turret's lock light already had
    // once. Inert on any machine that did not buy a conduit move.
    if (typeof drawConduitFX === 'function') drawConduitFX(c, this, cx);
    ...
    // ===================================================================
    // KINGDOM A — WHAT THE MEADOW PICKED UP, DRAWN.
    //
    // These are the telegraphs for scrapdrag / scrapkick / rustbloom, and they
    // live HERE, in front of the atlas call, for the same reason the guard's
    // plate and the breaker's waves do: the authored sheet has one pose per
    // machine, so anything the body is holding has to be drawn beside it or it
    // does not exist on screen. A hitbox with no picture is the exact thing the
    // telegraph law forbids.
    //
    // The reserved amber above them is already drawn by the shared wind-up
    // block below — this is the second channel, the thing itself, so the
    // warning is not carried by one hue alone.
    if (!this.dead && (this.drag0 > 0 || this.plate)) {
      // the plate: hooked up off the floor during the coil, then shoved.
      const k = this.plate ? 1 : this.drag0;
      const ox = cx + this.dir * (this.w * 0.5 + (this.plate ? DRAG_REACH * 0.5 : 4));
      const oy = this.y + this.h - 3 - k * (this.h * 0.55);
      c.save();
      c.translate(ox, oy);
      c.rotate(this.dir * (0.9 - k * 0.75));        // it comes up edge-on, then flat
      const pw = DRAG_REACH * (0.55 + k * 0.45), ph = 3 + k * 2;
      c.fillStyle = '#7d4a22';
      c.beginPath(); rr(c, -pw / 2, -ph / 2, pw, ph, 1.2); c.fill();
      c.fillStyle = '#b4702f';
      c.fillRect(-pw / 2, -ph / 2, pw, 1.1);        // the rust edge catching light
      c.strokeStyle = 'rgba(20,14,8,0.55)'; c.lineWidth = 0.6;
      c.beginPath(); rr(c, -pw / 2, -ph / 2, pw, ph, 1.2); c.stroke();
      c.restore();
      if (this.plate) {                              // it is a weapon now, not cargo
        c.save(); c.globalAlpha = 0.5;
        c.strokeStyle = TELL_COL; c.lineWidth = 1.6;
        c.beginPath(); c.moveTo(ox, this.y + 3); c.lineTo(ox, this.y + this.h - 2); c.stroke();
        c.restore(); c.globalAlpha = 1;
      }
    }
    if (!this.dead && this.rake0 > 0 && !this.loaded) {
      // the heap: scraped in under the springs while it crouches, and its SIZE
      // is the reach it is about to have. She is told how far before it goes.
      const k = this.rake0, hw = 10 + k * 16, hh = 2 + k * 4;
      c.save();
      c.fillStyle = '#7d4a22'; c.globalAlpha = 0.9;
      c.beginPath(); c.ellipse(cx, this.y + this.h - 1, hw, hh, 0, Math.PI, 0); c.fill();
      c.fillStyle = '#b4702f'; c.globalAlpha = 0.7;
      c.beginPath(); c.ellipse(cx, this.y + this.h - 1, hw * 0.6, hh * 0.7, 0, Math.PI, 0); c.fill();
      c.globalAlpha = 0.35 + k * 0.4;
      c.strokeStyle = TELL_COL; c.lineWidth = 1.4; c.setLineDash([4, 4]);
      c.lineDashOffset = -performance.now() / 60;
      for (const sd of [-1, 1]) {
        c.beginPath();
        c.moveTo(cx + sd * hw, this.y + this.h - 1);
        c.lineTo(cx + sd * KICK_REACH * k, this.y + this.h - 1);
        c.stroke();
      }
      c.setLineDash([]); c.restore(); c.globalAlpha = 1;
    }
    if (!this.dead && (this.washT || 0) > 0) {
      // the ring on the floor: grounded-only, so it is drawn ON the floor
      const a = clamp(this.washT * 2.4, 0, 1);
      c.save(); c.globalAlpha = a * 0.75;
      c.strokeStyle = '#d08a3c'; c.lineWidth = 2.4;
      for (const sd of [-1, 1]) {
        c.beginPath();
        c.moveTo(cx + sd * 4, this.y + this.h - 2);
        c.quadraticCurveTo(cx + sd * this.washR * 0.6, this.y + this.h - 9,
          cx + sd * this.washR, this.y + this.h - 2);
        c.stroke();
      }
      c.restore(); c.globalAlpha = 1;
    }
    if (!this.dead && this.bloom && this.drip0 > 0) {
      // THE BLOOM PAINTS WHERE, NOT MERELY THAT. Two amber marks on the two
      // tiles it is about to throw at, filling in as the sag runs — because a
      // hazard that lands where she is NOT standing has to name its ground or
      // the read is a guess. The sag itself is the puppet's (drip0 -> `sag`).
      c.save();
      for (const sd of [-1, 1]) {
        const mx = cx + sd * BLOOM_THROW, my = this.y + this.h - 2;
        c.globalAlpha = 0.25 + this.drip0 * 0.55;
        c.strokeStyle = TELL_COL; c.lineWidth = 1.6;
        c.setLineDash([4, 4]); c.lineDashOffset = -performance.now() / 70;
        c.beginPath(); c.ellipse(mx, my, 13, 4.5, 0, 0, 7); c.stroke();
        c.setLineDash([]);
        c.globalAlpha = this.drip0 * 0.5;
        c.fillStyle = '#b4702f';
        c.beginPath(); c.ellipse(mx, my, 13 * this.drip0, 4.5 * this.drip0, 0, 0, 7); c.fill();
      }
      c.restore(); c.globalAlpha = 1;
    }
    ...
    // =====================================================================
    // KINGDOM X'S THREE READS, drawn in world space because every one of
    // them threatens a place the body is not. The house rule is worn the
    // same way the other five wear it: AMBER while it is only a warning,
    // seam-blue once it is real — and the two anchored to the FLOOR are
    // suppressed under G.artProbe with every other ground FX so the bible
    // harness measures feet rather than what is lit under them.
    // ---------------------------------------------------------------------
    // FACET: the strip of floor behind the machine. Drawn at its TRUE extent
    // from the first frame of the coil — dashed amber, brightening with the
    // fill — so the ground it is about to sweep is legible before the body
    // has moved at all. Solid seam-blue only for the fifth of a second it is
    // really there, and frozen where the amber promised it would be.
    if (this.kind === 'crawler' && !this.dead && !G.artProbe
        && (this.facetT > 0 || (this.lanceT || 0) > 0)) {
      const live = (this.lanceT || 0) > 0;
      const fs = live
        ? { x: this.lanceX, y: this.lanceY, w: FACET_REACH, h: this.lanceH }
        : facetStrip(this);
      const k = live ? clamp(this.lanceT / FACET_LIVE, 0, 1) : this.facet0;
      c.save();
      if (!live) {
        c.globalAlpha = 0.16 + k * 0.4;
        c.strokeStyle = TELL_COL; c.lineWidth = 1.4;
        c.setLineDash([4, 6]); c.lineDashOffset = -performance.now() / 45;
        c.beginPath(); c.rect(fs.x, fs.y, fs.w, fs.h); c.stroke();
        c.setLineDash([]);
        // the facet itself, filling on the spine — the second channel, on the
        // body, so the machine that owns its own back is read from its shape
        c.globalCompositeOperation = 'lighter';
        c.globalAlpha = 0.35 + k * 0.5;
        c.fillStyle = TELL_COL; c.shadowColor = TELL_COL; c.shadowBlur = 7;
        c.beginPath();
        c.moveTo(cx - 5, this.y + 3);
        c.lineTo(cx, this.y - 1 - k * 4);
        c.lineTo(cx + 5, this.y + 3);
        c.closePath(); c.fill();
        c.shadowBlur = 0;
      } else {
        // the lance: a flat pane of split light lying on the floor it swept,
        // fading out across its own short life
        c.globalCompositeOperation = 'lighter';
        c.globalAlpha = 0.5 * k + 0.25;
        const lg = c.createLinearGradient(fs.x, fs.y, fs.x, fs.y + fs.h);
        lg.addColorStop(0, 'rgba(143,232,255,0)');
        lg.addColorStop(1, 'rgba(143,232,255,0.85)');
        c.fillStyle = lg;
        c.fillRect(fs.x, fs.y, fs.w, fs.h);
        c.globalAlpha = 0.6 * k + 0.3;
        c.strokeStyle = '#d6f6ff'; c.lineWidth = 1.5; c.lineCap = 'round';
        for (let i = 0; i < 4; i++) {
          const lx2 = fs.x + fs.w * (0.12 + i * 0.25);
          c.beginPath();
          c.moveTo(lx2, fs.y + fs.h);
          c.lineTo(lx2 + (i - 1.5) * 2.4, fs.y + fs.h * (0.15 + (1 - k) * 0.35));
          c.stroke();
        }
      }
      c.restore(); c.globalAlpha = 1;
    ...
    // ---------------------------------------------------------------------
    // REFRACT: the painted landing spot. This is the whole move — the arc is
    // allowed to lie precisely because the FLOOR does not — so it is lit from
    // the first frame of the crouch, stays lit for the entire flight, and
    // reads at a glance as a place rather than an effect: a caustic ellipse
    // with the crystal's own cross in it, pulsing on its own clock.
    if (this.kind === 'hopper' && !this.dead && !G.artProbe && this.markX != null) {
      const mk = 0.45 + this.bend0 * 0.55;
      const pulse = 0.72 + Math.sin(this.anim * 7) * 0.28;
      c.save();
      c.globalCompositeOperation = 'lighter';
      c.globalAlpha = 0.34 * mk * pulse;
      const mg = c.createRadialGradient(this.markX, this.markY - 2, 1, this.markX, this.markY - 2, 30);
      mg.addColorStop(0, '#ffd27a'); mg.addColorStop(1, 'rgba(255,210,122,0)');
      c.fillStyle = mg;
      c.beginPath(); c.ellipse(this.markX, this.markY - 2, 28, 10, 0, 0, 7); c.fill();
      c.globalCompositeOperation = 'source-over';
      c.globalAlpha = 0.35 + 0.45 * mk;
      c.strokeStyle = TELL_COL; c.lineWidth = 1.6;
      c.setLineDash([5, 5]); c.lineDashOffset = performance.now() / 55;
      c.beginPath(); c.ellipse(this.markX, this.markY - 2, 16 * mk + 6, 6 * mk + 2.5, 0, 0, 7);
      c.stroke(); c.setLineDash([]);
      // the cross: two facet lines through the mark, the picture that says
      // "light landed here" rather than "something is glowing"
      c.globalAlpha = 0.3 + 0.4 * mk;
      c.lineWidth = 1.2; c.lineCap = 'round';
      for (const a of [0.42, -0.42]) {
        c.beginPath();
        c.moveTo(this.markX - Math.cos(a) * 15, this.markY - 2 - Math.sin(a) * 5);
        c.lineTo(this.markX + Math.cos(a) * 15, this.markY - 2 + Math.sin(a) * 5);
        c.stroke();
      }
      c.restore(); c.globalAlpha = 1;
    }
    // ---------------------------------------------------------------------
    // GLINT: the splinters hanging where the dive passed. Not a ground FX —
    // the whole point is that they are in the AIR — so no artProbe guard, and
    // no shadow under them either. Amber, hollow and growing while they are
    // arming and cannot touch her; seam-blue, solid and sharp-edged for the
    // beat they can. Two different shapes, not two different brightnesses:
    // the read has to survive being glanced at.
    if (this.kind === 'bat' && this.motes && this.motes.length) {
      c.save();
      for (const g of this.motes) {
        if (g.arm > 0) {
          const k = 1 - clamp(g.arm / GLINT_ARM, 0, 1);
          c.globalAlpha = 0.25 + k * 0.5;
          c.strokeStyle = TELL_COL; c.lineWidth = 1.3;
          c.setLineDash([3, 4]); c.lineDashOffset = -performance.now() / 50;
          c.beginPath(); c.arc(g.x, g.y, GLINT_R * (0.35 + k * 0.65), 0, 7); c.stroke();
          c.setLineDash([]);
          continue;
        }
        const fade = clamp(g.life / 0.3, 0, 1);
        c.globalCompositeOperation = 'lighter';
        c.globalAlpha = 0.5 * fade;
        c.fillStyle = '#8fe8ff'; c.shadowColor = '#8fe8ff'; c.shadowBlur = 9;
        c.beginPath();
        for (let i = 0; i < 6; i++) {                 // a six-point splinter
          const a = i * Math.PI / 3 + this.anim * 0.8;
          const r = i % 2 ? GLINT_R * 0.42 : GLINT_R;
          c[i ? 'lineTo' : 'moveTo'](g.x + Math.cos(a) * r, g.y + Math.sin(a) * r);
        }
        c.closePath(); c.fill();
        c.shadowBlur = 0;
        c.globalCompositeOperation = 'source-over';
        c.globalAlpha = 0.9 * fade;
        c.strokeStyle = '#d6f6ff'; c.lineWidth = 1.1;
        c.stroke();
      }
      c.restore(); c.globalAlpha = 1;
    }
      ...
      case 'surge': {
        ...
        // drum below stays the stand-in while a plate is in flight.
        {
          ...
          const bDrew = typeof drawPlateAnchored === 'function' &&
            drawPlateAnchored(c, bp, 0, this.h / 2, this.h * (tell ? 1.35 : 1.1), false, true);
          c.restore();
          if (bDrew) break;
        }
...
function drawBoomer(c) {
  ...
  c.translate(b.x, b.y);
  c.rotate(b.spin);
  const art=typeof MEDIA_RAW!=='undefined' && MEDIA_RAW.heroJoinedProjectile;
  if(art && art.naturalWidth){
    const fx=MEDIA_RAW.swingDualFx;
    if(fx && fx.complete && fx.naturalWidth){
      const cw=fx.naturalWidth/8;
      c.save();c.globalCompositeOperation='lighter';c.globalAlpha=.55;
      c.drawImage(fx,3*cw,0,cw,fx.naturalHeight,-38,-38,76,76);c.restore();
    }
    // Rotation follows the actual projectile; collision, return and catch are unchanged.
    c.drawImage(art,-30,-30,60,60);
    c.restore();
    return;
  }
  if(typeof mediaFetch==='function')mediaFetch('heroJoinedProjectile',1);
