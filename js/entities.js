...
class Enemy {
  ...
  draw(c) {
    ...
    const steel = (y0, y1) => {
      const g = c.createLinearGradient(-6, y0, 6, y1);
      g.addColorStop(0, '#8496ad');    // lit plate, cool
      g.addColorStop(0.45, '#4d5a6b');
      g.addColorStop(1, '#1d242e');    // deep shadow, bluer still
      return g;
    };
    const plate = (path, y0, y1) => { c.fillStyle = steel(y0, y1); path(); c.fill(); };
    ...
    switch (this.kind) {
      case 'crawler': {
        ...
        c.fillStyle = '#59677a';                        // rim of the open bin
        ...
        // the coupling: an open jaw with a tooth, so the front end reads as a head
        c.fillStyle = '#2b333d';
        c.beginPath(); c.moveTo(-19.5, -1.4); c.lineTo(-14.5, -0.6);
        c.lineTo(-14.5, 2.4); c.lineTo(-19, 1.6); c.closePath(); c.fill();
        c.fillStyle = '#59677a';
        c.beginPath(); c.moveTo(-18.6, -0.8); c.lineTo(-16.2, -0.4); c.lineTo(-17.4, 1.2); c.closePath(); c.fill();
      ...
      case 'hopper': {
        ...
        c.strokeStyle = '#39434f'; c.lineWidth = 1.5; c.lineCap = 'round';
        for (const sx of [-4.5, 4.5]) {
          ...
          c.stroke();
          c.fillStyle = '#2b333d'; c.fillRect(sx - 2.6, 11, 5.2, 1.8);
        }
      ...
      case 'blob': {
        ...
        c.strokeStyle = '#2b333d'; c.lineWidth = 2.4; c.lineCap = 'round';
      ...
      case 'flier': {
        ...
        c.strokeStyle = '#4d5a6b'; c.lineWidth = 2.4;
        ...
        c.fillStyle = '#ff2f4f'; c.shadowColor = '#ff2f4f'; c.shadowBlur = 9;
        c.beginPath(); c.arc(-1.2, 0, 3.1, 0, 7); c.fill();
        c.shadowBlur = 0;
        c.fillStyle = '#20272f'; c.beginPath(); c.arc(-1.2, 0, 1.3, 0, 7); c.fill();
        c.fillStyle = 'rgba(255,255,255,0.55)';
        c.beginPath(); c.arc(-2.8, -1.6, 0.9, 0, 7); c.fill();
        // arc emitters underneath
        accent(() => {
          for (const ex of [-5, 5]) { c.beginPath(); c.arc(ex, 8.4, 1.3, 0, 7); c.fill(); }
        }, true);
        c.strokeStyle = '#4d5a6b'; c.lineWidth = 1;
      ...
      default: {
        ...
        c.fillStyle = '#2b333d';
        for (const bx of [-9.5, 9.5]) { c.beginPath(); c.arc(bx, 12.4, 1.5, 0, 7); c.fill(); }
        ...
        c.fillStyle = '#39434f'; c.fillRect(-13, -8.4, 8, 3.4);      // barrel
        accent(() => {                                                // arc coils
