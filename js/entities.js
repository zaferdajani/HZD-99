    ...
    switch (this.kind) {
      case 'glitch': {
        ...
        for (const [lx, ph] of [[-24, 0], [-11, 2.2], [12, 1.1], [25, 3.4]]) {
          ...
          c.beginPath(); c.moveTo(lx * 0.7, -4); c.lineTo(kx, ky); c.lineTo(kx + sw * 3, 24); c.stroke();
          c.strokeStyle = MAT.steel.mid; c.lineWidth = 2.4;
          c.beginPath(); c.moveTo(lx * 0.7, -4); c.lineTo(kx, ky); c.stroke();
      ...
      case 'atlas': {
        ...
        c.fillStyle = ramp(c, MAT.ceramic, -9, -30, 9, 22);
        rr(c, -10, -30, 20, 54, 4); c.fill();
        c.save();
        c.fillStyle = MAT.molten.deep; c.fillRect(-5, -26, 10, 46);
        const mg = c.createLinearGradient(0, -26, 0, 20);
        mg.addColorStop(0, MAT.molten.lit); mg.addColorStop(0.5, MAT.molten.mid); mg.addColorStop(1, MAT.molten.dark);
        c.globalAlpha = 0.55 + heat * 0.4 + Math.sin(tt * 6) * 0.05;
        c.shadowColor = MAT.molten.mid; c.shadowBlur = 10 + heat * 16;
        c.fillStyle = mg; c.fillRect(-4, -25, 8, 44);
        c.restore(); c.shadowBlur = 0;
        wear(c, [[-9, -6, 4], [6, 10, 3]]);
      ...
      case 'zero': {
        ...
        const st8 = sensorState(this);
        c.translate(0, fl);
        for (const [ox, k] of [[-1, 0.82], [1, 0.9], [0, 1]]) {
          c.fillStyle = ramp(c, MAT.ceramic, ox * 8 - 10, -16, ox * 8 + 12, 28, k);
          c.beginPath();
          c.moveTo(ox * 3 - 8, -14);
          c.bezierCurveTo(ox * 4 - 13, 4, ox * 6 - 20, 20, ox * 7 - 23, 29);
          c.lineTo(ox * 7 + 23, 29);
          c.bezierCurveTo(ox * 6 + 20, 20, ox * 4 + 13, 4, ox * 3 + 8, -14);
          c.closePath(); c.fill();
        }
        occl(c, 0, 28, 22, 4, 0.5);
