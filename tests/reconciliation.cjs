...
(async()=>{
 ...
 const evidence=await p.evaluate(()=>{
   ...
   // 1.90 -> 1.52 on the owner's 2026-09-18 instructions to show 15% then 20% more of
   ...
   check('latest smaller sizing is preserved, not inferred from test constants',PRESENTATION.explorationZoom===1.52&&PRESENTATION.heroVisualScale===HERO_SCREEN_SCALE,{actor:HERO_SCREEN_SCALE,presentation:PRESENTATION.heroVisualScale,zoom:PRESENTATION.explorationZoom});
