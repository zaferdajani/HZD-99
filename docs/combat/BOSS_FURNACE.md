...
| `fc.hymn` | `hymn` | **1000** | 2 (p1) / 3 (p2) expanding heat rings | **2000 / 3000** | `hymn` ✓ **(fixed this pass)** | 1000 | amber + bells winding + roar wave + rumble + cue | 1 core | **JUMP the ring as it passes / GUARD facing the bell / sing** — changed 2026-09-19: the ring takes only a GROUNDED body (`player.on`), and a facing brace holds (see HERO_ANSWERS §2); it fires at most every 6.5–8.5 s (`hymnCD`), lobbing when the cycle asks and the cooldown refuses; `hymn_warn` toast once per save | **1967 / 2967** |
