...
const files = require('./source-files.json').map(f => fs.readFileSync('js/' + f + '.js', 'utf8'));
...
const MUS_EXT=/\.(ogg|mp3|m4a|wav)$/i, musFiles={}; try{for(const f of fs.readdirSync('assets/music').sort()) if(MUS_EXT.test(f)) musFiles[f.replace(MUS_EXT,'')]='assets/music/'+f;}catch(e){}
const VID_EXT=/\.(mp4|webm|mov)$/i, vidFiles={},vidAlt={}; try{for(const f of fs.readdirSync('assets/video').sort()){if(!VID_EXT.test(f))continue;const b=f.replace(VID_EXT,'');if(/\.webm$/i.test(f))vidAlt[b]='assets/video/'+f;else vidFiles[b]='assets/video/'+f;}for(const k in vidAlt)if(!vidFiles[k]){vidFiles[k]=vidAlt[k];delete vidAlt[k];}}catch(e){}
let vidLight='null';try{vidLight=fs.readFileSync('assets/video/light/index.json','utf8').trim();}catch(e){}
const VOX_EXT=/\.(ogg|mp3|m4a|wav)$/i,voxFiles={};try{for(const f of fs.readdirSync('assets/vox').sort())if(VOX_EXT.test(f))voxFiles[f.replace(VOX_EXT,'')]='assets/vox/'+f;}catch(e){}
